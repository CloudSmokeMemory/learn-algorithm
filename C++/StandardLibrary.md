...
分类：output、input、forward、bidirectional、random-access
迭代器相关的辅助函数：advance()、next()、prev()、distance()、iter_swap()
迭代器适配器Iterator Adapter：Reverse迭代器、Insert迭代器、Front迭代器、General迭代器、stream迭代器、Move迭代器、
迭代器特性：迭代器可以区分为不同种类，每个类型都代表特定的迭代器能力，如果能够根据不同的迭代器种类将操作进行重载，很有用。
自定义迭代器
...
后缀：_if:参数要求传入一个函数或function object。_copy:在此算法中元素不止被操作还会复制到标的区间。
分类：非更易型(既不改动元素次序也不改动元素值)，更易型(要不是直接改变元素值，就是在复制元素到另一个 区间的过程中改变元素值)，移除型(可以移除区间内的元素，在复制过程中执行移除动作)，变序型(通过元素值的赋值和变换，改变元素顺序，但不改变元素值)，排序型(特殊的变序型)，已排序区间，数值。
